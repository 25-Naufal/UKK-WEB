...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['meja'])) {
        $nama= htmlentities($_POST['meja']);
        $id= htmlentities($_POST['id']);
        $data[] = $nama;
        $data[] = $id;
        $sql = 'UPDATE meja SET  nama_meja=? WHERE id_meja=?';
        $row = $config -> prepare($sql);
        $row -> execute($data);
        echo '<script>window.location="../../index.php?page=meja&uid='.$id.'&success-edit=edit-data"</script>';
    }
    ...
    if (!empty($_GET['barang'])) {
        ...
        $sql = 'UPDATE barang SET id_kategori=?, nama_barang=?,  
				 harga_jual=?, satuan_barang=?, stok=?, tgl_update=?  WHERE id_barang=?';
        $row = $config -> prepare($sql);
        $row -> execute($data);
